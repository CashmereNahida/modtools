    ...
    for (element_name, data) in sorted(vertex_layers.items()):
        ...
        for component in range(dim):
            ...
            if type(data[0][0]) == int:
                mesh.attributes.new(name=layer_name)
                layer = mesh.attributes[layer_name]
...
def blender_vertex_to_3dmigoto_vertex(mesh, obj, blender_loop_vertex, layout, texcoords):
    ...
    for elem in layout:
        ...
        if elem.name == 'POSITION':
            if 'POSITION.w' in attributes:
                attribute_data = attributes['POSITION.w'].data
                vertex[elem.name] = pos + [attribute_data[blender_loop_vertex.vertex_index].value]
            else:
            ...
            for component in 'xyzw':
                ...
                if layer_name in attributes:
                    attribute_data = attributes[layer_name].data
                    data.append(attribute_data[blender_loop_vertex.vertex_index].value)
                elif layer_name in attributes:
                    data.append(attribute_data[blender_loop_vertex.vertex_index].value)
            if data:
        ...
        if elem.name == 'POSITION':
            ...
            for component in 'xyzw':
                ...
                if layer_name in mesh.attributes:
                    data.append(mesh.attributes[layer_name].data[blender_loop_vertex.vertex_index].value)
                elif layer_name in mesh.attributes:
                    data.append(mesh.attributes[layer_name].data[blender_loop_vertex.vertex_index].value)
            if data:
                vertex[elem.name] = data
...
class Import3DMigotoFrameAnalysis(bpy.types.Operator, ImportHelper, IOOBJOrientationHelper):
    ...
    def convert_dds_to_png_with_alpha_check(self):
        dirname = os.path.dirname(self.filepath)
        dds_files = [f for f in os.listdir(dirname) if f.endswith('.dds') and 'Diffuse' in f and 'FaceHead' not in f]

        converted_textures = []
        for dds_file in dds_files:
            dds_path = os.path.join(dirname, dds_file)
            png_path = dds_path.replace('.dds', '.png')

            if os.path.exists(png_path):
                converted_textures.append(png_path)
                continue
            ...
            with Image.open(dds_path) as img:
                ...
                if 'A' in img.getbands():
                    ...
                    alpha_is_inverted = self.is_alpha_inverted(a)

                    if alpha_is_inverted:
                        a = a.point(lambda i: 255 - i)  # 알파 반전
    ...
    def is_alpha_inverted(self, alpha_channel):
        total_pixels = alpha_channel.size[0] * alpha_channel.size[1]
        transparent_pixels = sum(alpha_channel.point(lambda i: i < 128).getdata())

        return transparent_pixels / total_pixels > 0.5

    def assign_textures_to_materials(self, textures):
        for obj in bpy.data.objects:
            if obj.type == 'MESH':
                # Object name processing
                parts = obj.name.split('-')
                if len(parts) < 2:
                    continue

                character_name = parts[0]
                part_name = parts[1].split('=')[0].replace('vb0', '')

                # Find texture path
                texture_path = next((t for t in textures if character_name in t and part_name in t and 'Diffuse' in t), None)
                if not texture_path:
                    continue
                ...
                try:
                    ...
                    # Enable node use and clear existing nodes
                    mat.use_nodes = True
                    ...
                    logging.error(f"Error while assigning material to '{obj.name}': {e}")
    ...
    def execute(self, context):
        ...
        try:
            ...
            if 'auto_materials' in keywords:
                del keywords['auto_materials']
        ...
        if self.auto_materials:
            converted_textures = self.convert_dds_to_png_with_alpha_check()
            self.assign_textures_to_materials(converted_textures)
...
class Import3DMigotoRaw(bpy.types.Operator, ImportHelper, IOOBJOrientationHelper):
    ...
    def execute(self, context):
        ...
        if 'auto_materials' in migoto_raw_import_options:
            del migoto_raw_import_options['auto_materials']
...
class Export3DMigotoGenshin(bpy.types.Operator, ExportHelper):
    ...
    def set_outlines(self, texcoord_file_path, thickness):
        texcoord_file = [x for x in os.listdir(".") if "Texcoord.buf" in x]
        if len(texcoord_file) == 0:
            print("ERROR: unable to find texcoord file. Ensure you are running this in the same folder as CharTexcoord.buf. Exiting")
            return
        ...
        if len(ini_file) == 0:
            print("ERROR: unable to find .ini file. Ensure you are running this in the same folder as Char.ini. Exiting")
            return
        if len(ini_file) > 1:
        ...
        with open(texcoord_file, "rb+") as f:
            print("Adjusting outline thickness")
        ...
        print("Outline thickness adjustment complete")
    ...
    def execute(self, context):
        ...
        if self.auto_outlines:
            self.set_outlines(vb_path, self.outline_thickness)
